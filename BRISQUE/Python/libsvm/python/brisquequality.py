...
def computeBRISQUE(folderPath):
    scores = []
    imgs = {}
    for f in listdir(folderPath):
        if isfile(join(folderPath, f)):
            full_path = join(folderPath,f)
            qualityscore = test_measure_BRISQUE(full_path)
            scores.append(qualityscore)
            print("Score of the given image", full_path, ":", qualityscore)
            imgs[f] = qualityscore

    print("BRISQUE Avg = ", sum(scores)/len(scores))
    return sum(scores)/len(scores)

# computeBRISQUE("D:\F1(16mtr)")